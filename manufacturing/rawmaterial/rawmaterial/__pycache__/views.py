...
def add_mat(request,id):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            # product.total_manufactured_amount=product.total_charge_amount+product.raw_material_amount
            product.save()
...
def purchase_product(request,id):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            if unit_manu > price:
                y.status="loss"
                loss=float(unit_manu-price)
                y.price_pro_loss=loss
                y.save()
            elif  unit_manu < price:
                y.status="profit"
                prfit=float(price-unit_manu)
                y.price_pro_loss=prfit
                y.save()
